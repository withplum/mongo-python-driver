...
class TopologyDescription:
    ...
    def _apply_local_threshold(self, selection: Optional[Selection]) -> List[ServerDescription]:
        ...
        fastest = min(cast(float, s.round_trip_time) for s in selection.server_descriptions if s.round_trip_time is not None)
        threshold = self._topology_settings.local_threshold_ms / 1000.0
        return [
            s
            for s in selection.server_descriptions
            if s.round_trip_time is not None and (cast(float, s.round_trip_time) - fastest) <= threshold
        ]
